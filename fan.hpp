...
#include "meta/property_wrapper.hpp"
...
private:
	struct gauge_type_guard {
		...
		static bool check(const shared_ptr<SF> &, const shared_ptr<SF> &gauge) { return check(gauge.get()); }
	};

	struct valve_type_guard {
		static bool check(const pwm *valve);
		static bool check(const shared_ptr<pwm> &, const shared_ptr<pwm> &valve) { return check(valve.get()); }
	};
...
public:
	...
	class label_wrapper: public meta::const_property_wrapper<std::string, meta::guards::old_empty<std::string> > {
		friend class config;
	}
	m_label;

	class gauge_wrapper: public meta::property_wrapper<shared_ptr<SF>, gauge_type_guard> {
	public:
